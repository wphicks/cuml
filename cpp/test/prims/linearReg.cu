...
class LinRegLossTest : public ::testing::TestWithParam<LinRegLossInputs<T>> {
 protected:
  void SetUp() override
  {
    ...
    std::vector<T> h_out_grad_ref_vec{-0.56995, -3.12486};
    ...
    std::vector<T> h_out_lasso_grad_ref_vec{0.03005, -3.724866};
    ...
    std::vector<T> h_out_ridge_grad_ref_vec{-0.14995, -3.412866};
    ...
    std::vector<T> h_out_elasticnet_grad_ref_vec{-0.05995, -3.568866};
